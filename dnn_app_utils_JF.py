...
def predict_deep_xxx(parameters, X):
    """
    ...
    # Computes floats using forward propagation, and rounds them
    floats, caches = L_model_forward_regressor(X, parameters)    
    predictions = np.round(floats)
...
def calculate_accuracy_deep_dnn(parameters, X_train, X_test, Y_train, Y_test):
    
    # Predict test/train set examples 
    Y_prediction_train = predict_deep(parameters, X_train)
    Y_prediction_test = predict_deep(parameters, X_test)

    #convert to boolean vector containing 'True' for prediction == groundtruthk
    idx_train_bool = Y_prediction_train == Y_train
    idx_test_bool = Y_prediction_test == Y_test
    
    #convert from boolean (true/false) to integer (1/0)
    idx_train = idx_train_bool.astype(int) 
    idx_test = idx_test_bool.astype(int)
    
    #calculate accuracy in % as mean of ones
    Y_train_accuracy = np.mean(idx_train)*100
    Y_test_accuracy = np.mean(idx_test)*100
...
def regressor_output(Z):
    """
    Perform linear regression on output to get one-unit outputlayer calculation
    
    Arguments:
    Z -- numpy array of any shape
    
    Returns:
    A -- output of regressor_output(Z), same shape as Z
    cache -- returns Z as well for backpropagation
    """
...
def L_model_forward_regressor_dnn(X, parameters):
    """
    Implement forward propagation for the [LINEAR->RELU]*(L-1)->LINEAR->SIGMOID computation
...
def L_model_backward_regressor_dnn(AL, Y, caches):
    """
    Implement the backward propagation for the [LINEAR->RELU] * (L-1) -> LINEAR -> SIGMOID group
    ...
    # Lth layer ONLY LINEAR gradients. Inputs: "AL, Y, caches". Outputs: "grads["dAL"], grads["dWL"], grads["dbL"]
    current_cache = caches[L-1]
...
def predict_moon(X, y, parameters):
    """
